...
class Player:
    ...
    def __init__(self, size: pygame.Vector2 = pygame.Vector2(30, 50), acceleration: float = 2, air_acceleration: float = 1, deceleration: float = 0.8, air_deceleration: float = 0.9, jump_power: float = -15, gravity_power: float = 1.2, jump_gravity_power: float = 0.6):
        ...
        self.level_file = json.load(open("assets/levels.json"))
        self.level_list = []

        for i in self.level_file:
            self.level_list.append(json.load(open(i["level_path"])))
            print("Player: " + i["level_name"] + " successfully loaded")
    ...
    def tick(self, level_id: int, camera: Camera):
