...
def render_level(surface: pygame.surface, level_id: int, camera: gameplay.Camera, resolution: tuple[int, int], player: player.Player):
    file = level_list[level_id]
    surface.fill(((level_file[level_id])["background_color"]))
    for i in file:
         if i["type"] == 0:
             if i["texture"] == "none":
                 pygame.draw.rect(surface, (255, 255, 255), (((i["position_x"] - (i["width"] / 2)) - camera.position.x) + resolution[0] / 2, ((i["position_y"] - (i["height"] / 2)) - camera.position.y) + resolution[1] / 2, i["width"], i["height"]))
             else:
                 texture = texture_list[i["texture"]]
                 surface.blit(pygame.transform.scale(texture, (i["width"], i["height"])), (((i["position_x"] - (i["width"] / 2)) - camera.position.x) + resolution[0] / 2, ((i["position_y"] - (i["height"] / 2)) - camera.position.y) + resolution[1] / 2))
